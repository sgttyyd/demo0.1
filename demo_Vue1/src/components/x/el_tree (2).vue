    ...
    <el-container>
      ...
      <el-container>
        ...
        <el-aside width="200px" v-show="show3">          
          ...
          -->
          <ul id="btnList">
            <li data-path="//a.amap.com/amap-ui/static/data/restaurant.json">
              海沃润扬物业
            </li>
            <li data-path="//a.amap.com/amap-ui/static/data/hotel.json">
              名扬物业
            </li>
          </ul>
    ...
    init () {
      ...
      AMapUI.loadUI(['misc/MarkerList', 'overlay/SimpleMarker', 'overlay/SimpleInfoWindow'],
        function (MarkerList, SimpleMarker, SimpleInfoWindow) {
          ...
          var $btns = $('#btnList li[data-path]')
          /**
           * 检测各个button的状态
           */
          function checkBtnStats () {
            $('#btnList li[data-enable]').each(function () {
              var $li = $(this)
              var codeEval = $li.attr('data-enable')
              $li.prop({
          ...
          $('#btnList').on('click', 'li', function () {
            var $li = $(this)
            var dataPath = $li.attr('data-path')
            var codeEval = $li.attr('data-eval')
            if (dataPath) {
              loadData(dataPath)
            } else if (codeEval) {
              eval(codeEval)
